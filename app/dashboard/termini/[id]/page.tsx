...
export default async function TerminDetaljPage({
  ...
  const psc = {
    ...
    refunded:   { label: 'Refundirano',cls:'bg-[rgba(255,255,255,0.05)] text-[#8A93BC]' },
  } as const
        ...
        <div className="mt-6 flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[#A0A8C8] text-sm mt-1">
              ...
              {daysLeft > 0 && <span className={`ml-2 font-medium ${daysLeft <= 3 ? 'text-[#F59E0B]':'text-[#8A93BC]'}`}>({daysLeft === 1 ? 'sutra' : `za ${daysLeft} dana`})</span>}
            </p>
          </div>
        ...
        <div className="grid lg:grid-cols-[1fr_340px] gap-5 items-start mt-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#13162A] border border-[#1C2040] rounded-[16px] p-5">
              <p className="text-xs uppercase tracking-wide text-[#8A93BC] font-semibold mb-3">Link za plaćanje</p>
              <div className="bg-[#0D0F1A] rounded-md px-3 py-2 flex items-center justify-between gap-2 mb-3">
                <a href={paymentUrl} target="_blank" rel="noreferrer" className="flex-1 min-w-0 text-[#8B6FFF] hover:text-[#A78BFF] text-sm font-mono truncate transition flex items-center gap-1.5">
                  {paymentUrl}
                  <svg aria-hidden="true" className="w-4 h-4 shrink-0 text-[#8A93BC]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                </a>
              </div>
            ...
            <div className="bg-[#13162A] border border-[#1C2040] rounded-[16px] p-5">
              <div className="flex justify-between items-center mb-3">
                <span className="text-xs uppercase tracking-wide text-[#8A93BC] font-semibold">Napredak</span>
            ...
            <div className="bg-[#13162A] border border-[#1C2040] rounded-[16px] p-5 grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-xs uppercase tracking-wide text-[#8A93BC] font-semibold mb-1">Gost plaća</p>
            ...
            {/* Payments table */}
            <div className="mt-6">
              <h2 className="text-xs uppercase tracking-wide text-[#8A93BC] font-semibold mb-3">Uplate ({payments?.length ?? 0})</h2>
              {!payments || payments.length === 0 ? (
                <div className="bg-[#13162A] border border-dashed border-[#1C2040] rounded-[16px] p-10 text-center">
                  ...
                  <p className="text-[#8A93BC] text-xs mt-1">Podijeli link ili QR kod</p>
                </div>
              ) : (
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-[#1C2040] bg-[#0D0F1A]">
                          ...
                          <th className="text-left px-4 py-3 text-xs font-semibold text-[#8A93BC] uppercase tracking-wide hidden sm:table-cell">Email</th>
                          <th className="text-right px-4 py-3 text-xs font-semibold text-[#8A93BC] uppercase tracking-wide">Iznos</th>
                          <th className="text-center px-4 py-3 text-xs font-semibold text-[#8A93BC] uppercase tracking-wide">Status</th>
                          <th className="text-right px-4 py-3 text-xs font-semibold text-[#8A93BC] uppercase tracking-wide hidden sm:table-cell">Datum</th>
                        </tr>
                      </thead>
                      <tbody>
                        {payments.map((p, idx) => {
                          const s = psc[p.status as keyof typeof psc] ?? { label: p.status, cls:'bg-[rgba(255,255,255,0.05)] text-[#8A93BC]' }
                          ...
                          return (
                            <tr key={p.id} className="border-b border-[#1C2040] last:border-0 hover:bg-[rgba(255,255,255,0.02)] transition">
                              ...
                              <td className="px-4 py-3 text-right text-[#8A93BC] text-xs hidden sm:table-cell">{new Date(p.created_at).toLocaleDateString('hr-HR')}</td>
                            </tr>
                          )
                        })}
                      </tbody>
